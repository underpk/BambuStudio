...
void TabPrint::build()
{
    ...
    page = add_options_page(L("Support"), "support");
        ...
        optgroup->append_single_option_line("tree_support_base_pattern", "support#base-pattern");
...
wxSizer* Tab::compatible_widget_create(wxWindow* parent, PresetDependencies &deps)
{
    ...
    deps.checkbox->Bind(wxEVT_CHECKBOX, ([this, &deps](wxCommandEvent e)
    {
        ...
        // Auto-save non-system presets so compatible_printers persists across sessions.
        // System presets are read-only and require "Save As" with a new name.
        if (m_presets->current_is_dirty() && !m_presets->get_selected_preset().is_system) {
            m_presets->save_current_preset(m_presets->get_edited_preset().name);
            this->update_tab_ui();
        }
    }) );
    ...
    deps.btn->Bind(wxEVT_BUTTON, ([this, parent, &deps](wxCommandEvent e)
    {
        ...
        if (dlg.ShowModal() == wxID_OK) {
            ...
            this->update_changed_ui();
            // Auto-save non-system presets so compatible_printers persists
            if (m_presets->current_is_dirty() && !m_presets->get_selected_preset().is_system) {
                m_presets->save_current_preset(m_presets->get_edited_preset().name);
                this->update_tab_ui();
            }
